...
%begin{table}[htbp!]
    ...
    \caption{符号约定}\label{tab:001}\\
        ...
        $d$ & 鼓与球相碰时鼓的位移\\
        \hline
        $\overrightarrow{F}$ & 人拉绳子的力\\
        \hline
        $\overrightarrow{L}$ & $\overrightarrow{L}$的力矩\\
        \hline
        $\overrightarrow{L_0}$ & $\overrightarrow{L}$的初始力矩\\
        \hline
        $\theta$ & 绳子与竖直方向夹角\\
        \hline
        $t$ & $\overrightarrow{F}$的作用时间\\
        \hline
        $\overrightarrow{\omega}$ & 角速度\\
        \hline
        $I$ & 转动惯量\\
        \bottomrule[1.5pt]
%\end{table}
...
\section{模型假设}
在描述我们建立的模型之前，有必要把我们做出的一些模型上的假设一一阐明，并简要地分析一下我们做出如下假设的合理性。
\begin{enumerate}
	\item 在本题的条件下，空气阻力的影响可以忽略不计。
	\item 所处环境较为封闭，无风力扰动等外界因素干扰。
	\item 鼓是刚体。
	\item 在问题1中的理想条件下，每个人都可以精确控制用力方向、时机和力度。
	\item 在问题2求解中：
	\begin{itemize}
		\item 当鼓面与排球发生斜碰时，可以忽略摩擦力和排球旋转情况。
...
\begin{figure}[!h]
    ...
    \label{fig:p3}
\end{figure}
列出力的矢量表达式，之后再用空间解析几何知识计算力臂\footnote{Wolfram alpha提供了方便的计算和API，可见网址：https://www.wolframalpha.com本文不罗列计算数据，因为实在过于繁琐}，代入公式(\ref{eq:eps})，易算得转动角度
\end{enumerate}
