...
\begin{figure}[h]
	\centering
	\includegraphics[width=.7\textwidth]{jz.pdf}
	\caption{精确控制模拟}
	\label{fig:jz}
\end{figure}

\subsubsection{模型概述}
如图\ref{fig:jz}所示，矩形为倾斜的鼓，排球下方箭头是其下落与弹射时初速度的方向。假设此处球与鼓为弹性碰撞（事实上这与我们之前的假设相悖，
但此处给出的方向偏离非常小，非弹性碰撞造成的法向速度损失便小到可忽略不计），由位移偏移1°易知初速度偏移为0.5°，
因此图中鼓身倾斜角度$\beta=0.25°$. 此时排球已以与竖直方向偏离0.5°的初速度斜抛而出。
\subsubsection{角度校正}
为使得下一次碰撞后排球恰能竖直方向落下，调整后的鼓面倾角应为相反方向倾斜0.25°即$\beta = -0.25°$. 
假设排球如图\ref{fig:jz}所示碰撞后的瞬间，
鼓静止且受力平衡，现需一与排球弹出方向处同一竖直面上的合力使鼓身向反方向旋转0.5°. 供调整的时间恰是球弹起后下落到原高度的总时间：
$$t=\sqrt{\frac{2h}{g}}=0.35s$$
为便于队员们操作，不妨设校正所用合力仅持续0.05s，鼓在此$t_1=0.05s$内匀加速转动，在剩下的$t_2=0.3s$内匀速转动，总共转的角度为$2\beta$.
因此有方程:
\begin{equation}
\left\{
\begin{array}{lr}
	\overrightarrow{r}\times \overrightarrow{F^{'}}t_1 = I\omega \\
	\frac{1}{2}\omega t_1+\omega t_2 = 2\beta
\end{array}
\right.
\label{eq:jz}
\end{equation}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=.6\textwidth]{fj.pdf}
	\caption{受力分解}
	\label{fig:fj}
\end{figure}

解得$F^{'}=0.0015N$. 现将所需合力$F^{'}$分解为两位邻近两位队员所持绳方向的力即$F_1$与$F_2$, 如图\ref{fig:fj}所示。
解得$F_1=0.001N$, $F_1=0.00053N$. 由于鼓在当前受力下保持平衡，因此两位邻近的队员只需在已发力基础上再分别增加$F_1$与$F_2$大小的力即可。
即1:2的夹角中角度为1的队员增加0.00053N的用力，另一位队员增加0.001N的用力，持续用力0.05s后恢复原力，即可使排球再次碰撞时恰能再次竖直弹起。
\subsubsection{可行性分析}
由前所述队员们发力基本在50~90N间，$F_1$与$F_2$相对来说微不足道，在现实中对用力的控制也很难达到如此精度。另外，现实中也很难满足鼓在合力下静止且保持平衡状态等一系列假设。因此，我们认为球偏离的1°情况不用进行额外的调整策略，事实上1°的偏离也很难从肉眼看出，
故此校正模型可行性并不大。
...
右为排球速度随时间变化的曲线。此处排球与鼓都沿z轴方向位移，遂取z轴向上方向为正。从图上可看出排球与鼓做同步的周期运动，
右图速度图像是图\ref{vt}在t正半轴上的完美周期性延拓。由于浮点误差排球弹起高度并非每次都达到40cm，
但即使不足时也与40cm的高度要求相差不到2cm，很好的说明了模型结果的正确性。
